...
const Dashboard: React.FC = () => {
  ...
  const handleDeleteInterview = useCallback(
    async (e: React.MouseEvent, interviewId: number) => {
      e.stopPropagation();
      if (
        !window.confirm(
          "Are you sure you want to delete this interview and all its data?",
        )
      ) {
        return;
      }
  ...
  const stats = useMemo(() => {
    ...
    const avgScore =
      scored.length > 0
        ? Math.round(
            scored.reduce((sum, i) => sum + (i.overallScore || 0), 0) /
              scored.length,
          )
        : null;
    const lastInterview =
          ...
          >
            <CardContent sx={{ p: 3 }}>
              ...
              <Typography
                variant="h3"
                fontWeight={700}
                sx={{ color: getScoreColor(stats.averageScore ?? undefined) }}
              >
      ...
      <GridAny container spacing={3} sx={{ mb: 4 }}>
        ...
        <GridAny size={{ xs: 12, sm: 4 }}>
          <Card
            ...
            onClick={() => navigate("/communication-live")}
          >
            <CardContent
              sx={{ p: 3, display: "flex", alignItems: "center", gap: 2 }}
            >
              ...
              <Box>
                ...
                <Typography variant="body2" sx={{ opacity: 0.85 }}>
                  Improve English with real-time AI conversation
                </Typography>
              </Box>
            </CardContent>
          </Card>
          ...
          >
            ...
            <Box sx={{ display: "flex", alignItems: "center", ml: 1 }}>
              ...
              <IconButton
                ...
                color="error"
                onClick={(e) =>
                  handleDeleteInterview(e, interview.interviewId!)
                }
